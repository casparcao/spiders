...
def extract_and_normalize_apis(text):
    ...
    # 匹配 $http.method("path") 格式，注意路径可能不完整
    # 支持单引号、双引号、反引号
    pattern = r'\$http\.(get|post|put|delete|patch)\s*\(\s*["\'`]([^"\'`]*?)["\'`]'
...
def main():
    input_file = 'http_get_contents.txt'
    output_file = 'normalized_apis3_fixed.xlsx'  # 更改文件名避免冲突
